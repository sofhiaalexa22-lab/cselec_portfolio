...
class FacialRecognitionSystem {
  ...
  async detectFaceData(source) {
    if (this.useFaceApi) {
      return await faceapi.detectSingleFace(source, new faceapi.TinyFaceDetectorOptions()).withFaceLandmarks().withFaceExpressions().withFaceDescriptor();
    }
  ...
  async captureAndVerify() {
    ...
    try {
      ...
      const resultsHTML = `
        ...
        <p>Your face has been verified with smile and blink detection. You may now proceed to change your pin number.</p>
  ...
  async confirmVerification(face) {
    ...
    try {
      if (face.descriptor) {
        ...
        const response = await fetch('http://localhost:3000/recover', {
          ...
          body: JSON.stringify({faceData: Array.from(descriptor)})
        });
        const result = await response.json();
        console.log('Identity stored:', result);
      } else {
        console.log('Face descriptor not available');
      }
    } catch (error) {
      console.error('Error storing identity:', error);
    }

    // Show results section and continue button
    this.resultsSection.style.display = 'block';
    ...
    if (this.continueBtn) {
      ...
      alert('Continue button should now be visible below the camera. Click it to proceed.');
    } else {
      alert('Continue button not found!');
    }
